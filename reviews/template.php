...
$reviews = $connection->query($query)->fetchAll();
...
//        echo $i % 12 . "     -----      " . $firstReviewIndex . '       --------      ' . count($reviews) . '<br>';
        ...
        if ($reviews[$i]['type_file'] == 'image') {
            $reviewsHTML .= '<img class="card-review__image card-review__content" src="./img/screens/' . $reviews[$i]['id_review'] . '_' . $reviews[$i]['name_file'] . '" alt="review screen">';
        } else {
            $reviewsHTML .= '<video src="./video/' . $reviews[$i]['id_review'] . '_' . $reviews[$i]['name_file'] . '" class="card-review__video card-review__content"></video>
